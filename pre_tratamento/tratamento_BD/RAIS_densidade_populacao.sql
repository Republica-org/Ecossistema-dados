WITH tabela_1 AS (
   SELECT a.*, b.populacao AS populacao_mun,  FROM
    `basedosdados.br_me_rais.microdados_vinculos`   AS a
      INNER JOIN `basedosdados.br_ibge_populacao.municipio` AS b 
      ON a.ano = b.ano and a.id_municipio = b.id_municipio ),


  tabela_2 AS (

  SELECT a.*, b.populacao as populacao_uf
   
   FROM
      tabela_1 AS a
      INNER JOIN `basedosdados.br_ibge_populacao.uf` AS b 
      ON a.ano = b.ano and a.sigla_uf = b.sigla_uf),

  tabela_3 AS (

  SELECT a.*, b.populacao as populacao_br
   
   FROM
      tabela_2 AS a
      INNER JOIN `basedosdados.br_ibge_populacao.brasil` AS b 
      ON a.ano = b.ano),
 ...
 SELECT 
  ...
  populacao_uf,
  populacao_br,
  SUM(populacao_uf) OVER (partition by nome_regiao,ano) AS populacao_regiao
...
GROUP BY 
  ...
  (100 * SUM(quantidade_vinculos) OVER (partition by id_municipio,ano)/populacao_mun) AS densidade_vin_munic, 
  
  (100 * SUM(quantidade_vinculos) OVER (partition by sigla_uf,ano)/populacao_uf) AS densidade_vin_uf,

  (100 * SUM(quantidade_vinculos) OVER (partition by nome_regiao,ano)/populacao_regiao) AS densidade_vin_regiao,

  (100* SUM(quantidade_vinculos) OVER (partition by ano)/populacao_br) AS densidade_vin_br,
