-- Etapa 1: CTE otimizada, selecionando apenas as colunas necessárias e resolvendo ambiguidades
WITH tabela_1 AS (
  SELECT
    ...
    b.faixa_etaria,
    b.indicador_portador_deficiencia,
...
-- Etapa 2: Consulta final com as classificações e a agregação correta
SELECT
  ano,
  sigla_uf,
  nome_municipio,
  ...
  -- Decodificação de Grau de Instrução
  CASE
    WHEN ano >= 2006 THEN (
      CASE
  ...
  -- Decodificação do Indicador de Deficiência
  CASE
...
FROM
  tabela_1
WHERE
  ((natureza_juridica NOT LIKE "1%" OR natureza_juridica = '1228') AND natureza_juridica NOT IN ('2011', '2038'))
  AND vinculo_ativo_3112 = '1' -- CORRIGIDO para comparar como STRING

GROUP BY
  -- O GROUP BY agora corresponde exatamente às colunas não agregadas do SELECT
  ano,
  sigla_uf,
  nome_municipio,
  id_municipio,
