-- Etapa 1: Criar uma CTE para filtrar os dados da RAIS UMA ÚNICA VEZ.
WITH rais_vinculos_publicos_filtrada AS (
  SELECT
    ano,
    natureza_juridica,
    sexo,
  ...
  WHERE
    ...
    AND vinculo_ativo_3112 = '1' -- Ajustado para STRING
)

-- Etapa 2: Calcular a média de remuneração para cada variável, usando a tabela já filtrada.
...
FROM
  rais_vinculos_publicos_filtrada
GROUP BY
  ano,
  variavel,
  categoria
...
-- 3. Análise por Grau de Instrução
SELECT
  ano,
  ...
  CASE
    ...
    WHEN ano < 2006 THEN (
      CASE
        WHEN grau_instrucao_1985_2005 IN ('1', '2', '3', '4', '5', '6') THEN 'Até Fundamental' -- Ajustado para ser consistente
...
-- 4. Análise por Nível de Governo
SELECT
  ano,
