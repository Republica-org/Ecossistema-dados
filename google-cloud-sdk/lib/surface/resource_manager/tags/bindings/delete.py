...
class Delete(base.Command):
  ...
  def Run(self, args):
    ...
    if args.tag_value.find("tagValues/") == 0:
      ...
      tag_value_id = args.tag_value
      try:
        tag_value = tag_utils.GetNamespacedResource(
            tag_value_id, tag_utils.TAG_VALUES
        ).name
        ...
        if (
          ...
          if tag_value_id.count("/") == 2:
            tag_key_namespaced, unused_value_short_name = tag_value_id.rsplit(
                "/", 1
            )
            tag_key = tag_utils.GetNamespacedResource(
                tag_key_namespaced, tag_utils.TAG_KEYS
            )
            if (
                hasattr(tag_key, "allowedValuesRegex")
                and tag_key.allowedValuesRegex
            ):
              tag_value = tag_key.name
            else:
              raise e
          else:
            raise core_exceptions.Error(
                "Namespaced tag values should be of the format "
                "{parent_namespace}/{tag_key_short_name}/{tag_value_short_name}."
            )
        else:
          raise e
      ...
      with endpoints.CrmEndpointOverrides(location):
        ...
        if args.async_ or op.done:
          ...
          return operations.WaitForReturnOperation(
              op,
              "Waiting for TagBinding for resource [{}] and tag value [{}] to"
              " be deleted with [{}]".format(args.parent, tag_value, op.name),
          )
